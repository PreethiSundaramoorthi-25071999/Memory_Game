...
function initGame() {
    const gameBoard = document.getElementById('game-board'); //get the game board container
...
function handleCardClick(event){
       ...
       if(flippedCards.length === 2 || clickedCard.classList.contains('flip')) {
           return; //exit the fucntion if conditions are met
       }       

       //flip the card by adding the 'flip' class
       clickedCard.classList.add('flip');
       flippedCards.push(clickedCard); //add the clicked card to the flipped array

       //check if two cards have flipped
...
function checkForMatch() {
    ...
    if(card1.dataset.symbol === card2.dataset.symbol){
        ...
        //check if the all pairs have been matched
        if(matchedPairs === cardsymbols.length){
            setTimeout(() => alert('You Rocked! Click on restart to play again.'), 500); //display the win message after the short delay
        }
    } else {
...
//event listener for the restart buttion to restart the game
